...
int configure_lsm330tr(int test)
{
    ...
    if(!test) // set normal configuration
    {
        ...
        if(lsm330_write_reg(LSM330_DEV_ACCEL, LSM330_ACC_OFFX, OFFSETX_A) < 0) return -1;
        if(lsm330_write_reg(LSM330_DEV_ACCEL, LSM330_ACC_OFFY, OFFSETY_A) < 0) return -1;
        if(lsm330_write_reg(LSM330_DEV_ACCEL, LSM330_ACC_OFFZ, OFFSETZ_A) < 0) return -1;
    }
